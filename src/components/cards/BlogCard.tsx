...
function BlogCard({ blog }: BlogCardProps) {
  return (
    <Link href={`/blog/${blog.slug?.current}`} target='_blank' title={`Blog: ${blog.title}`}>
        <div className='bg-background rounded-2xl space-y-5 border-2 hover:border-black transition duration-300 h-full flex flex-col'>
            {blog.mainImage && (
                <div className='p-1'>
            ...
            <div className='p-5 pt-0 space-y-5 flex flex-col h-full'>
                <p className='font-medium text-lg line-clamp-2'>{blog.title}</p>
                <p className='flex gap-2 items-center mt-auto'>Zobacz bloga <ExternalLink className='size-5' /></p>
            </div>
        </div>
    </Link>
  )
